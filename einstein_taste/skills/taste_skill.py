...
The skill provides:
- evaluate_taste: Evaluate a theory against Einstein's taste
- rank_theories: Rank multiple theories
- compare_theories: Compare two theories
- get_taste_axes: Get information about Einstein's taste axes
...
from einstein_taste.agents.taste_agent import TasteAgent
from einstein_taste.config.settings import EINSTEIN_TASTE_AXES, EINSTEIN_PERIODS
...
class EinsteinTasteSkill:
    """
    Claude Code skill wrapper for Einstein's research taste modeling.
    ...
    def evaluate_taste(self, theory: str, cutoff_year: int = 1955) -> dict:
        """
        Evaluate how a scientific theory aligns with Einstein's research taste.

        Args:
            theory: Description of the theory or scientific question
            cutoff_year: Year to evaluate Einstein's taste at (avoids anachronism)

        Returns:
            Dict with overall score, per-axis scores, and evidence-based explanations
        """
        from einstein_taste.utils.formatting import evaluation_to_dict
    ...
    def rank_theories(self, theories: list[str], cutoff_year: int = 1955) -> list[dict]:
        """
        Rank multiple theories by their alignment with Einstein's taste.
    ...
    def get_taste_axes(self) -> list[dict]:
        """Get information about Einstein's taste axes."""
        return [
            {
                "name": a.name,
            ...
            for a in EINSTEIN_TASTE_AXES
        ]

    def get_periods(self) -> list[dict]:
        """Get Einstein's career periods and their dominant taste axes."""
        return [
            {
                "name": p.name,

...
Einstein's documented research taste axes.
...
from einstein_taste.config.settings import (
    EINSTEIN_TASTE_AXES,
    EINSTEIN_PERIODS,
    TasteAxis,
    TemporalPeriod,
...
class TasteEvaluation(BaseModel):
    """Complete evaluation of a candidate theory/question against Einstein's taste."""
    ...
    def evidence_based_score(self) -> float:
        ...
        if not eb_scores:
            return 0.0
        axes_by_name = {a.name: a for a in EINSTEIN_TASTE_AXES}
...
class TasteScorer:
    """
    Scores candidate theories/questions against Einstein's taste axes.
    ...
    def __init__(self, evidence_store: EvidenceStore):
        ...
        self.axes = {a.name: a for a in EINSTEIN_TASTE_AXES}
        self.periods = {p.name: p for p in EINSTEIN_PERIODS}

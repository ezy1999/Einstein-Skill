...
against Einstein's taste axes, grounded in retrieved historical evidence.
...
from einstein_taste.config.settings import EINSTEIN_TASTE_AXES, ModelConfig, DEFAULT_CONFIG
from einstein_taste.core.evidence import EvidenceRecord, SourceType
...
EVALUATOR_SYSTEM_PROMPT = """You are an expert historian of science specializing in Albert Einstein's
scientific methodology, philosophy, and research taste. Your task is to evaluate
how a candidate scientific question or theory aligns with Einstein's documented
...
2. You MUST clearly distinguish between what is supported by evidence and what
   you are inferring.
3. You MUST respect the temporal cutoff: Einstein at year {cutoff_year} would NOT
   know about developments after that year.
4. You MUST NOT engage in role-playing. You are analyzing Einstein's documented
   preferences, not pretending to be Einstein.
...
- Historical evidence about Einstein's preferences
...
def format_evidence_for_prompt(records: list[tuple[EvidenceRecord, float]]) -> str:
    ...
    for record, relevance in records:
        source_label = {
            SourceType.PRIMARY: "[PRIMARY - Einstein's own words]",
...
class TasteEvaluator:
    """
    LLM-based evaluator that scores candidates against Einstein's taste axes.
    ...
    def evaluate(
        ...
        """
        Evaluate a candidate theory/question against Einstein's taste.
        ...
        user_prompt = (
            ...
            f"## Temporal Cutoff\n\nYear: {cutoff_year}. Einstein at this point "
    ...
    def _get_period_name(self, year: int) -> str:
        ...
        for period in EINSTEIN_PERIODS:
